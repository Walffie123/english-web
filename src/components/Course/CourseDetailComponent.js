                ...
                <div className={cx('col-md-8', 'lesson-container')}>
                    {lesson.map((lesson, index) => (
                        ...
                        <div className={cx('lesson')} key={index}>
                            <Card.Title className={cx("cardtitle")} >Lesson {index + 1}: {lesson.lessonName}</Card.Title>
                            <Button variant="primary" href={`findLesson/${lesson.lessonId}`}>Go to lesson</Button>
                        </div>
                        </Card.Body>
                        </Card>
                    ))
